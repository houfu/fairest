from fairest.core.logging import init_logger, fairest_logger
...
def fairest(request: Request) -> Response:
    ...
    fairest_logger.debug(
        ...
        f'options: f{request.options if request.options else "Default"}')
    ...
    if document:
        ...
        fairest_logger.debug(
            f'Document model details: Document Type: f{document.document_type}; '
            f'Number of sections: f{len(document)}')
